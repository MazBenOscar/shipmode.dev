...
export async function initProject(templateName: string): Promise<void> {
  ...
  if (!await fs.pathExists(templatePath)) {
    const available = await fs.readdir(path.join(TEMPLATES_DIR, 'init')).catch(() => [] as string[]);
    throw new Error(
      `Init template "${templateName}" not found.\nAvailable: ${available.join(', ') || 'minimal'}`
    );
  }
